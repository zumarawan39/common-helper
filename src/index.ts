 ...
 */
export const truncateId = (id: string, startLength = 3, endLength = 4): string => {
...
export const truncate = (
  ...
  if (appendPosition === 'center') {
    ...
    if (text.length <= startLength) return text;
    const start = text.slice(0, startLength);
 ...
 */
export const getDateRange = (filter: number): DateRange => {
 ...
 */
export const buildFilters = (filters: any) => {
 ...
 */
export const generateCoupon = (options: CouponOptions = {}): string => {
  const {
    length = 8,
    prefix = '',
    suffix = '',
...
export const generateRandomPassword = (length: number = 6): string => {
  ...
  const randomChar = (min: number, max: number) =>
    charset[Math.floor(Math.random() * (max - min + 1)) + min];
